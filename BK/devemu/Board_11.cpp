...
void CMotherBoard_11::StopInterrupt()
{
	// нажали на кнопку стоп
	if (!(m_reg177662out & 0100000)) // если кнопка СТОП не заблокирована
	{
		m_cpu.SetIRQ1();
	}
}

void CMotherBoard_11::UnStopInterrupt()
{
	// отжали кнопку стоп
	if (!(m_reg177662out & 0100000)) // если кнопка СТОП не заблокирована
	{
...
int CMotherBoard_11::GetScreenPage() const
{
...
*/
bool CMotherBoard_11::SetSystemRegister(uint16_t addr, uint16_t src, UINT dwFlags)
{
	...
	switch (addr & 0177776)
	{
		case 0177660:
			...
			*/
			mask = dwFlags & GSSR_INTERNAL ? 0300 : 0100;
