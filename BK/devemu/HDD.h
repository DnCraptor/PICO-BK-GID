...
class CHDD
{
		...
		fs::path m_strImageFileName; // имя файла образа
		...
		int                 m_nSectorCounter;  // т.к. 0 в регистре 1f2 означает 256 секторов, то чтобы такое работало
		...
		bool identifydevice();
	public:
		CHDD(const fs::path &name, HDD_MODE mode);
		CHDD();
		virtual ~CHDD();
		bool attach_hdd(const fs::path &name, HDD_MODE mode);
		void detach_hdd();
		...
		uint16_t get_debug_regs(HDD_REGISTER reg, bool bReadMode) const;
};
...
class CATA_IDE
{
		...
		CHDD m_mDrive[2];
	public:
		bool attach(const fs::path &name, HDD_MODE mode = HDD_MODE::MASTER);
		void detach(HDD_MODE mode = HDD_MODE::MASTER);
		void reset();
		void write_regs(HDD_REGISTER reg, uint16_t data);
		uint16_t read_regs(HDD_REGISTER reg);
		uint16_t read_regs_internal(HDD_REGISTER reg) const;

		uint16_t get_debug_regs(int nDrive, HDD_REGISTER reg, bool bReadMode) const;
};
...
The only two values used by this command are the Sector Count
