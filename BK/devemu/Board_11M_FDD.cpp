// Board_11M_FDD.cpp: implementation of the CMotherBoard_11M_FDD class.
//
...
{
	return MSF_CONF::BK11M_FDD;
}

void CMotherBoard_11M_FDD::SetFDDType(BK_DEV_MPI model, bool bInit)
{
...
BK_DEV_MPI CMotherBoard_11M_FDD::GetFDDType()
{
	return m_fdd.GetFDDType();
}

uint8_t *CMotherBoard_11M_FDD::GetAddMemory() const
{
	return GetMainMemory() + 0640000;
}

void CMotherBoard_11M_FDD::OnReset()
{
...
/*
Запись данных в системные регистры БК0011M
...
*/
bool CMotherBoard_11M_FDD::SetSystemRegister(uint16_t addr, uint16_t src, UINT dwFlags)
{
	if (CMotherBoard_11M::SetSystemRegister(addr, src, dwFlags))
	{
		return true;
	}
...
bool CMotherBoard_11M_FDD::GetSystemRegister(uint16_t addr, void *pDst, UINT dwFlags)
{
	if (CMotherBoard_11M::GetSystemRegister(addr, pDst, dwFlags))
	{
		return true;
	}
...
bool CMotherBoard_11M_FDD::InitMemoryModules()
{
	bool bRes = CMotherBoard_11M::InitMemoryModules();
...
void CMotherBoard_11M_FDD::MemoryManager()
{
	CMotherBoard_11M::MemoryManager(); // сперва восстановим память в окне 1
	...
	for (int i = 014, bnk = BRD_11_BOS_BNK; i <= 015; ++i, ++bnk)
	{
		...
		m_MemoryMap[i].nTimingCorrection = 2;
	}
...
bool CMotherBoard_11M_FDD::Interception()
{
	if (CMotherBoard_11M::Interception())
	{
		return true;
	}
...
bool CMotherBoard_11M_FDD::RestoreMemory(CMSFManager &msf)
{
	if (CMotherBoard_11M::RestoreMemory(msf))
	{
