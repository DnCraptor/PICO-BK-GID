...
BEGIN_MESSAGE_MAP(CSprWnd, CFrameWnd)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_COMMAND(IDC_BUTTON_COLOR_MODE, &CSprWnd::OnViewColormode)
	ON_COMMAND(IDC_BUTTON_BW_MODE, &CSprWnd::OnViewBWMode)
	ON_CBN_SELENDOK(IDC_COMBO_PALETTE, &CSprWnd::OnSetPalette)
	ON_CBN_SELENDOK(IDC_COMBO_SCALE, &CSprWnd::OnSetScale)
END_MESSAGE_MAP()
...
int CSprWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	int nIndex = m_wndToolBar.CommandToIndex(IDC_BUTTON_COLOR_MODE);
	m_wndToolBar.SetButtonStyle(nIndex, m_wndToolBar.GetButtonStyle(nIndex) | TBBS_CHECKBOX);
	nIndex = m_wndToolBar.CommandToIndex(IDC_BUTTON_BW_MODE);
	m_wndToolBar.SetButtonStyle(nIndex, m_wndToolBar.GetButtonStyle(nIndex) | TBBS_CHECKBOX);
	nIndex = m_wndToolBar.CommandToIndex(IDC_COMBO_SCALE);
	m_wndToolBar.SetButtonInfo(nIndex, IDC_COMBO_SCALE, TBBS_SEPARATOR, ::MulDiv(38, nPixelW, DEFAULT_DPIX));
	CRect rect;
	m_wndToolBar.GetItemRect(nIndex, &rect);

	if (!m_comboBoxScale.Create(CBS_DROPDOWNLIST | CBS_SIMPLE | WS_VISIBLE |
	                            WS_TABSTOP | WS_VSCROLL, rect, &m_wndToolBar, IDC_COMBO_SCALE))
	{
	...
	nIndex = m_wndToolBar.CommandToIndex(IDC_COMBO_PALETTE);
	m_wndToolBar.SetButtonInfo(nIndex, IDC_COMBO_PALETTE, TBBS_SEPARATOR, ::MulDiv(98, nPixelW, DEFAULT_DPIX));
	m_wndToolBar.GetItemRect(nIndex, &rect);

	if (!m_comboBoxPalette.Create(CBS_DROPDOWNLIST | CBS_SIMPLE | WS_VISIBLE |
	                              WS_TABSTOP | WS_VSCROLL, rect, &m_wndToolBar, IDC_COMBO_PALETTE))
	{
...
{
	int nBtnIndex = m_wndToolBar.CommandToIndex(IDC_BUTTON_COLOR_MODE);
	...
	nBtnIndex = m_wndToolBar.CommandToIndex(IDC_BUTTON_BW_MODE);
