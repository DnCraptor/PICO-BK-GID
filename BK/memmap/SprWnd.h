...
class CSprWnd : public CFrameWnd
{
		...
		std::unique_ptr<uint8_t[]> m_pFileBuffer; // сюда будет читаться файл, из него будет формироваться текстура, чтобы вывести её DibDrawом
		size_t          m_nFileLen;
	...
	public:
		CSprWnd(std::unique_ptr<uint8_t[]> buffer, const size_t size);
		virtual ~CSprWnd() override;

		inline const size_t GetFileLen() const
		{
			return m_nFileLen;
		}
