                                            ...
                                            int  nBtnWidth)
{
	...
	if (!cBitmap.Attach(LoadImage(AfxFindResourceHandle(MAKEINTRESOURCE(uToolBar), RT_BITMAP),
	                              MAKEINTRESOURCE(uToolBar), IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE | LR_CREATEDIBSECTION))
	...
	auto        rgb     = (RGBTRIPLE *)(bmBitmap.bmBits);
...
{
	auto pnmtb = reinterpret_cast<LPNMTOOLBAR>(pNMHDR);
	...
	for (int i = 0; i < m_lstDropDownButton.GetSize(); i++)
	{
		...
		if (DropDownInfo.uButtonID == UINT(pnmtb->iItem))
		{
			...
			CRect rc;
			SendMessage(TB_GETRECT, (WPARAM)pnmtb->iItem, reinterpret_cast<LPARAM>(&rc));
			ClientToScreen(&rc);
