...
bool CBKImage::ReadCurrentDir(CBKImage::ItemPanePos pp)
{
	...
	if (m_pFloppyImage->ReadCurrentDir())
	{
		...
		{
			std::wstring str;
			m_pListCtrl->InsertItem(item, fr.strName.c_str());  // сюда можно добавить иконку
			...
			if (fr.nAttr & FR_ATTR::DIR)
			{
				...
				switch (fr.nRecType)
				{
					...
					case BKDirDataItem::RECORD_TYPE::LINK:
						str = g_strLink;
						break;
				...
				m_pListCtrl->SetItemText(item, CBKListCtrl::LC_TYPE_POS, str.c_str());
				str.clear();
				m_pListCtrl->SetItemText(item, CBKListCtrl::LC_BLK_SIZE_POS, str.c_str());
				m_pListCtrl->SetItemText(item, CBKListCtrl::LC_ADDRESS_POS, str.c_str());
				m_pListCtrl->SetItemText(item, CBKListCtrl::LC_SIZE_POS, str.c_str());
			}
			else
			{
				// теперь выведем тип записи
				switch (fr.nRecType)
				{
					case BKDirDataItem::RECORD_TYPE::LOGDSK:
						str = L"LOG";
						break;

					case BKDirDataItem::RECORD_TYPE::FILE:
						str = L"";
						break;
				...
				m_pListCtrl->SetItemText(item, CBKListCtrl::LC_BLK_SIZE_POS, buff);
				_snwprintf(buff, 32, L"%o\0", fr.nAddress);
				m_pListCtrl->SetItemText(item, CBKListCtrl::LC_ADDRESS_POS, buff);
				_snwprintf(buff, 32, L"%o\0", fr.nSize);
				m_pListCtrl->SetItemText(item, CBKListCtrl::LC_SIZE_POS, buff);
			}
			...
			m_pListCtrl->SetItemText(item, CBKListCtrl::LC_ATTR_POS, str.c_str());
			BKDirDataItem *nfr = std::addressof(fr);

			if (m_pFloppyImage->HasSpecificData())
			{
				if (fr.nRecType == BKDirDataItem::RECORD_TYPE::UP)
				{
					str.clear();
				}
				else
				...
				m_pListCtrl->SetItemText(item, CBKListCtrl::LC_SPECIFIC_POS, str.c_str());
			}

			m_pListCtrl->SetItemData(item, reinterpret_cast<DWORD_PTR>(nfr));
		}

		m_pListCtrl->SortItems(CBKListCtrl::MyCompareProc, reinterpret_cast<DWORD_PTR>(m_pListCtrl));
		m_pListCtrl->SetFocus();
		m_pListCtrl->SetItemState(pp.nFocusedItem, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED);
		m_pListCtrl->SetHotItem(pp.nFocusedItem);

		// и делаем верхний элемент обратно верхним
		if (pp.nTopItem > 0)
		{
			CRect r;
			m_pListCtrl->GetItemRect(pp.nTopItem, &r, LVIR_BOUNDS);
			m_pListCtrl->Scroll(CSize(0, pp.nTopItem * r.Height()));
		}

		// и ещё убедимся, чтобы выделенный элемент был видимым
		if (!m_pListCtrl->EnsureVisible(pp.nFocusedItem, FALSE))
		{
			CRect r;
			m_pListCtrl->GetItemRect(pp.nFocusedItem, &r, LVIR_BOUNDS);
			m_pListCtrl->Scroll(CSize(0, r.Height()));
		}

		OutCurrFilePath();
		std::wstring str = m_pFloppyImage->GetImageInfo();
...
void CBKImage::ViewSelected(const bool bViewAsText)
{
	...
	if (uSelectedCount > 0)
	{
		...
		for (size_t i = 0; i < uSelectedCount; ++i)
		{
			...
			{
				ViewFileAsSprite(fr); // проверка, файл это или нет, делается внутри.
			}
		}
	}
}
...
l_fatal:            // если фатальная - то просто выведем сообщение.
				...
				{
					if (fr->nRecType == BKDirDataItem::RECORD_TYPE::DIR)
					{
...
ADDOP_RESULT CBKImage::AddObject(const fs::path &findFile, bool bExistDir)
{
	...
	if (fs::is_directory(findFile))
	{
		...
		AFR.nRecType = BKDirDataItem::RECORD_TYPE::DIR;
...
{
	fr->strName = L"..";
	fr->nRecType = BKDirDataItem::RECORD_TYPE::UP;
