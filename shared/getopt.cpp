...
{
	assert(longopts != nullptr);
	char *nameend;
	size_t namelen;
	const struct option_a *p;
	const struct option_a *pfound = nullptr;
	int n_options;
	int option_index = 0;
			...
			if (!strncmp(p->name, d->__nextchar, namelen))
			{
				         ...
				         || pfound->val != p->val)
				{
					if (!ambig_fallback)
					{
						...
						{
							if ((ambig_set = reinterpret_cast<unsigned char *>(malloc(n_options * sizeof(char)))) == nullptr)
							{
								ambig_fallback = 1;
							}
...
static int process_long_option_w(int argc, wchar_t **argv, const wchar_t *optstring, const struct option_w *longopts, int *longind, int long_only, struct _getopt_data_w *d, int print_errors, const wchar_t *prefix)
{
	...
	const struct option_w *pfound = nullptr;
	int n_options;
	int option_index = 0;
			...
			if (!wcsncmp(p->name, d->__nextchar, namelen))
			{
				         ...
				         || pfound->val != p->val)
				{
					if (!ambig_fallback)
					{
						...
						{
							if ((ambig_set = reinterpret_cast<wchar_t *>(malloc(n_options * sizeof(wchar_t)))) == nullptr)
							{
								ambig_fallback = 1;
							}
	...
	if (pfound == nullptr)
	{
		if (!long_only || argv[d->optind][1] == L'-' || wcschr(optstring, *d->__nextchar) == nullptr)
		{
			if (print_errors)
			{
...
#define NONOPTION_P (argv[d->optind][0] != L'-' || argv[d->optind][1] == L'\0')
	...
	if (d->__nextchar == nullptr || *d->__nextchar == L'\0')
	{
		if (d->__last_nonopt > d->optind)
		{
			d->__last_nonopt = d->optind;
...

	{
		...
		if (temp[0] == L'W' && temp[1] == L';' && longopts != nullptr)
		{
			if (*d->__nextchar != L'\0')
			{
				d->optarg = d->__nextchar;
